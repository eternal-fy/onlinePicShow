...
func UploadSingMulFile(root string) {
	...
	r.POST("/fileHandle", func(c *gin.Context) {
	...
	r.POST("/fileHandle", func(c *gin.Context) {
		...
		for _, file := range files {
			...
			if err := c.SaveUploadedFile(file, "./"+file.Filename); err != nil {
				c.String(http.StatusBadRequest, fmt.Sprintf("fileHandle err %s", err.Error()))
				return
			}
		}
		c.String(200, fmt.Sprintf("fileHandle ok %d files", len(files)))
	})
	//默认端口号是8080
	r.Run(":8000")
}
